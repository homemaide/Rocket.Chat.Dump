				...
				for (const u of this.users.users) {
					...
					Meteor.runAsUser(startedByUserId, () => {
						...
						if (existantUser) {
							...
							const userId = Accounts.createUser({ email: u.email, password: u.username });
							Meteor.runAsUser(userId, () => {
								Meteor.call('setUsername', u.username, { joinDefaultChannelsSilenced: true });
								RocketChat.models.Users.setName(userId, u.name);
								RocketChat.models.Users.update({ _id: userId }, { $addToSet: { importIds: u.id }, $set: { requirePasswordChange: true } });
								u.rocketId = userId;
							});
						}
