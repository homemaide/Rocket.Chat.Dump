...
Template.RocketSearch.onCreated(function() {
	...
	const _search = () => {
		...
		if (this.scope.text.get()) {
			...
			eval('var text = \'/^([\\\\s\\\\S])*(' + this.scope.text.get()+'){1,}([\\\\s\\\\S])*$/i\'');
			//^([\s\S])*(儿){1,}([\s\S])*$/i
			//var text = this.scope.text.get();
			console.log(text);
			Meteor.call('rocketchatSearch.search', text , {rid:Session.get('openedRoom'), uid:Meteor.userId()}, _p, (err, result) => {
				if (err) {
